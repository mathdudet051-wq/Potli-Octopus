...
export default function SeaweedGarden() {
  ...
  const plant = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const emoji = SEAWEED_EMOJIS[Math.floor(Math.random() * SEAWEED_EMOJIS.length)];
    const height = 40 + Math.random() * 50;
    setPlants((prev) => {
      const next = [...prev, { id: Date.now(), emoji, x, height }];
  ...
  if (won) return <WinScreen message="Your ocean garden is amazing!" score={`${GOAL} plants grown!`} onReset={reset} />;
  ...
  return (
    <div className="max-w-md mx-auto text-center">
      <p className="text-white text-xl font-bold mb-2" style={{ fontFamily: "'Fredoka One', cursive" }}>
        Plant {GOAL} seaweeds! ({plants.length}/{GOAL})
      </p>
      {plants.length >= 7 && plants.length < GOAL && (
        <p className="text-green-300 text-lg font-bold mb-2" style={{ fontFamily: "'Fredoka One', cursive" }}>
      ...
      <div data-testid="garden" onClick={plant}
        ...
        style={{ height: 360, background: "linear-gradient(180deg, #0096c7 0%, #023e8a 60%, #c2a15f 100%)" }}>
        {plants.map((p) => (
          <div key={p.id} className="absolute bottom-4 flex flex-col items-center pointer-events-none"
            ...
            {Array.from({ length: Math.ceil(p.height / 12) }).map((_, i) => (
              <span key={i} className="text-2xl leading-none animate-sway" style={{ animationDelay: `${i * 0.1}s` }}>
                {p.emoji}
              </span>
            ))}
          </div>
        ...
        {plants.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center">
            <p className="text-white/60 text-xl text-center px-6" style={{ fontFamily: "'Fredoka One', cursive" }}>
              Tap the sandy bottom to plant seaweed!
            </p>
          </div>
        )}
      </div>
