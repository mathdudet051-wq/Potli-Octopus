...
export default function CoralBuilder() {
  const { gameT } = useLang();
  const gm = gameT.coralBuilder as any;
  ...
  const plant = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    setCorals((prev) => {
      const next = [...prev, { id: Date.now(), emoji, x, y, size: 28 + Math.random() * 20 }];
  ...
  if (won) return <WinScreen message={gm.winMessage} score={gm.winScore(corals.length)} onReset={reset} />;
    ...
    <div className="max-w-md mx-auto">
      <div className="text-center mb-3">
        <p className="text-white text-xl font-bold mb-2" style={{ fontFamily: "'Fredoka One', cursive" }}>
          {corals.length}/{GOAL} — {gm.instruction}
        </p>
      ...
      <div data-testid="reef-area" onClick={plant}
        className="relative w-full rounded-3xl overflow-hidden cursor-pointer border-4 border-white/40 select-none"
        style={{ height: 360, background: "linear-gradient(180deg, #023e8a 0%, #0077b6 40%, #1a5276 80%, #c2a15f 100%)" }}>
        {corals.map((c) => (
          <span key={c.id} className="absolute select-none pointer-events-none animate-growIn"
        ...
        {corals.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center">
            <p className="text-white/70 text-xl text-center px-6" style={{ fontFamily: "'Fredoka One', cursive" }}>{gm.instruction}</p>
          </div>
        )}
        {corals.length >= 7 && corals.length < GOAL && (
          <div className="absolute top-3 left-1/2 -translate-x-1/2 bg-green-400/80 text-white px-4 py-1 rounded-full text-sm font-bold"
            style={{ fontFamily: "'Fredoka One', cursive" }}>{gm.growing}</div>
        )}
      </div>
      <style>{`
